...
- [packages/engine/src/pipeline/generateChapter.ts:1-494](file://packages/engine/src/pipeline/generateChapter.ts#L1-L494)
...
The heart of the enhanced system lies in its sophisticated scene planning capabilities. The ScenePlanner component transforms high-level chapter objectives into detailed scene blueprints, ensuring each scene serves the story's dramatic needs while maintaining narrative flow.
...
**Diagram sources**
- [packages/engine/src/agents/scenePlanner.ts:1-228](file://packages/engine/src/agents/scenePlanner.ts#L1-L228)
- [packages/engine/src/types/index.ts:118-152](file://packages/engine/src/types/index.ts#L118-L152)

The planner employs adaptive scene counting based on story progress, generating 3-7 scenes depending on narrative position within the overall arc. Each scene is carefully crafted to advance plot, develop characters, and maintain appropriate tension levels.
...
Pipeline->>Planner : planScenes()
Planner->>Planner : calculateSuggestedSceneCount()
Planner->>Planner : buildScenePrompt()
Planner-->>Pipeline : ScenePlan
loop For each scene
...
**Diagram sources**
- [packages/engine/src/pipeline/generateChapter.ts:41-355](file://packages/engine/src/pipeline/generateChapter.ts#L41-L355)
...
The scene generation process represents the core innovation of the enhanced system, moving from chapter-level abstraction to granular scene composition:

```mermaid
flowchart TD
Start([Start Scene Generation]) --> LoadContext["Load Story Context<br/>- Story Bible<br/>- Current State<br/>- Previous Summaries"]
LoadContext --> DirectorDirection["Story Director Analysis<br/>- Tension Analysis<br/>- Objective Generation<br/>- Scene Structure Guidance"]
DirectorDirection --> ScenePlanning["Scene Planning<br/>- Adaptive Scene Count<br/>- Location Selection<br/>- Character Composition<br/>- Tension Distribution"]
...
SceneWriting --> Validation["Scene Validation<br/>- Canon Compliance<br/>- Quality Standards<br/>- Structural Integrity"]
...
This workflow ensures each scene serves multiple narrative functions: advancing plot, developing characters, maintaining tension, and contributing to world-building while preserving logical consistency.
...
- **Causes**: LLM output formatting issues, token limits exceeded
- **Solutions**: Enable fallback mechanisms, adjust temperature settings, increase max tokens

**Tension Management Problems**
- **Symptoms**: Inconsistent tension progression or unrealistic dramatic arcs
- **Causes**: Incorrect tension calculations or state synchronization issues
- **Solutions**: Verify tension controller configuration, check story state updates
...
- **Solutions**: Optimize scope windows, implement caching strategies, monitor resource usage
